#include <QtGui>
#include "grupocurves.h"
#include "ui_grupocurves.h"
#include <direct.h>
#include "BitaC/BitaC.h"
#include "Settings/Paths.h"

GrupoCurves::GrupoCurves(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::GrupoCurves)
{
    ui->setupUi(this);
...
//    connect(shrdMem,SIGNAL(varNotFound(QString)),this,SLOT(anuncmnt(QString)));
    ...
    ui->table4->setColumnCount(5);
    QStringList headers;
    headers << "VARIABLE"<<"TIPO"<<"DESCRIPCIÓN"<<"VMIN"<<"VMAX";
...
void GrupoCurves::openFile()
{
//    mkdir(user.toAscii().data());
//    mkdir(QString("%0%1/GruposCurvas").arg(ruta).arg(user).toAscii().data());
    QString fileName = QFileDialog::getOpenFileName(this,"",QString("%0%1\\GruposCurvas").arg(ruta).arg(user));
...
void GrupoCurves::readFromFile(QString fileName)
{
    QFile file(fileName);
...
QStringList GrupoCurves::rTrStringList()
{
    return StringList;
}

void  GrupoCurves::saveGroup(QString fileName)
{
    QFile file(fileName);
    ...
    //out << rTrStringList();
    out << StringList;
    BitaC::escribeBTC("Grupo de curvas creado");
}

void GrupoCurves::saveFile()
{
...
//    mkdir(QString("%0%1/GruposCurvas").arg(ruta).arg(user).toAscii().data());
    QString fileName = QString("%0%1\\GruposCurvas\\%2%3").arg(ruta).arg(user).arg(ui->rootD->text()).arg(".txt");
    saveGroup(fileName);
    close();
}

void GrupoCurves::saveFileAs()
{
    QString fileName = QFileDialog::getSaveFileName(this,"",QString("%0%1\\GruposCurvas").arg(ruta).arg(user));
    if (!fileName.isEmpty()) {
        fileName.append(".txt");
        saveGroup(fileName);
        close();
...
void GrupoCurves::saveFileE()
{
    ...
    out << StringListE;
    close();
    BitaC::escribeBTC("Grupo de curvas editado");
}

void  GrupoCurves::verifyVariable()
{
...
void  GrupoCurves::showVartoChange(int row,int column)
{
    if(ui->table2->rowCount()>=10){
        QMessageBox::information(this, tr(""),QString("El máximo de curvas son 10"));
        return;
    }
...
void  GrupoCurves::deleteVar(int row,int column)
{
  Row=row;
  Column=column;
...
void  GrupoCurves::showVariables(QString system)
{
    ...
    while (nlines < lines.size()) {
        ...
        if(line.count("\t",Qt::CaseSensitive)<4){
                    QMessageBox::information(this, tr("Error"),tr("Los datos del archivo '%0LV_G_%1%2' son incorrectos").arg(ruta).arg(system).arg(".txt"));
                    //return;
                }
        ...
        for(int i=0;i<=4;i++){
            ...
            QTableWidgetItem *newItem2 = new QTableWidgetItem(line.split(QString("\t")).at(i));
            if(!i){
                newItem->setFlags(Qt::ItemIsEnabled);
            ...
            ui->table3->setItem(nlines,i,newItem2);
        }
        nlines++;
...
QStringList GrupoCurves::rFrStringList(QString file)
{
...
void  GrupoCurves::showVariablesE()
{
...
void  GrupoCurves::showVartoChangeE(int row,int column)
{

    if(ui->table4->rowCount()>=10){
        QMessageBox::information(this, tr(""),QString("El máximo de curvas son 10"));
        return;
    }
...
void  GrupoCurves::deleteVarE(int row,int column)
{
    Row=row;
    Column=column;
...
QString  GrupoCurves::ftoa(float f)
{
...
//Anuncio
void  GrupoCurves::anuncmnt(QString strings)
{
    qDebug()<<strings;
    close();
    QMessageBox::information(this, tr("Error"),
